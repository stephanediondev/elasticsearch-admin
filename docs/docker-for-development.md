...
docker run --name elasticsearch8 -v elasticsearch8:/usr/share/elasticsearch/data -p 800:9200 -p 1800:9300 -e "cluster.name=elasticsearch" -e "node.name=docker" -e "xpack.security.enabled=true" -e "discovery.type=single-node" docker.elastic.co/elasticsearch/elasticsearch:8.0.0-alpha1
```
